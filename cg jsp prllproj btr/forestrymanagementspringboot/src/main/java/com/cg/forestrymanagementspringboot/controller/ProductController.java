...
import com.cg.forestrymanagementspringboot.service.ProductService;
...
public class ProductController {
	...
	@PostMapping(path = "/add-product", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
			...
			public ForestryResponse viewProduct(@PathVariable("productId") String productId) {
				ForestryResponse response = new ForestryResponse();
			...
			@PutMapping(path="/update-product/{productId}",produces=MediaType.APPLICATION_JSON_VALUE,
					consumes = MediaType.APPLICATION_JSON_VALUE)
			public  ForestryResponse updateProduct (@PathVariable("productId") String productId,@RequestBody ProductBean bean) {
				ForestryResponse response=new ForestryResponse();
				if(service.updateProduct(productId,bean)) {
					response.setStatusCode(201);
					response.setMessage("success");
