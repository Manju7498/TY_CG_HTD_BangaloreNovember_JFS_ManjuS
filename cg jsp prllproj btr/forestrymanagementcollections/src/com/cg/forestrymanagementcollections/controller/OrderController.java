		...
		while (true) {
			...
			case 1:
				try {
					...
					boolean check = services.addOrder(Integer.parseInt(orderNo), order);
					if (check) {
			...
			case 3:
				...
				try {
					int orderNo = scanner.nextInt();
					if (order != null) {
						OrderBean orderBean1 = services.getOrder(orderNo);
			...
			case 4:
				try {
					HashMap<Integer, OrderBean> order1 = services.getAllOrders();
					if (order1 != null) {
						System.out.println("Order Details : ");
						for (Entry<Integer, OrderBean> orderbean : order1.entrySet()) {
							System.out.println("Contract No : " + orderbean.getKey());
			...
			case 5:
				...
				try {
					int orderNo = scanner.nextInt();
					HashMap<Integer, OrderBean> order2 = services.getAllOrders();
					boolean isUpdate = false;
					for (Entry<Integer, OrderBean> order3 : order2.entrySet()) {
						if (order3.getKey() == orderNo) {
							isUpdate = true;
						}
					}
					if (isUpdate) {
						...
						order.setCustomerId(Integer.parseInt(newCustomerId));
						...
						order.setProductId(Integer.parseInt(newProductId));
						...
						order.setHaulierId(Integer.parseInt(newHaulierId));
						System.out.println("enter delivery date(yyyy-mm-dd)");
